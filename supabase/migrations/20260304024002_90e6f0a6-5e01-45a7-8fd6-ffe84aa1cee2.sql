...
-- 1. Drop SELECT policy on user_ai_api_keys to prevent client-side plaintext key access
DROP POLICY IF EXISTS "Users can view their own API keys" ON public.user_ai_api_keys;

-- 2. Recreate get_feature_ai_config with auth.uid() guard
